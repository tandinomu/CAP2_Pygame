...
class Game:
   ...
   def new_game(self):
       self.tic_tac_toe = TicTacToe(self)
       self.buttons = [
           Button(WIN_SIZE // 2 - 50, WIN_SIZE - 50, 100, 50, 'Restart', (0, 255, 0), self.new_game),
           Button(WIN_SIZE // 2, WIN_SIZE - 50, 100, 50, 'Pause', (255, 255, 0), self.pause_game),
           Button(WIN_SIZE // 2 + 50, WIN_SIZE - 50, 100, 50, 'Quit', (255, 0, 0), self.quit_game)
       ]

class Button:
   def __init__(self, x, y, width, height, text, color, onclickFunction=None):
       self.x = x
       self.y = y
       self.width = width
       self.height = height
...
class TicTacToe:
    def __init__(self, game):
        self.game = game
        self.field_image = self.get_scaled_image(path='resources/grid.jpg', res=[WIN_SIZE] * 2)
